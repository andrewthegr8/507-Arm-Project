...
ColorResult_t COLOR_SENSOR_Read(tcs34725_handle_t *handle) {
    uint16_t r, g, b, c;
    if (tcs34725_read_rgbc(handle, &r, &g, &b, &c) != 0) return COLOR_UNKNOWN;
    if (c < MIN_CLEAR) return COLOR_UNKNOWN;

    COLOR_SENSOR_PrintRGB(r, g, b, classify(rf, gf, bf));
...
static uint8_t my_i2c_deinit(void) {
...
/* static void my_debug_print(const char *fmt, ...) {
    char buf[128];
    va_list args;
    va_start(args, fmt);
